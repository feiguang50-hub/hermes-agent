...
CURATOR_REVIEW_PROMPT = (
    ...
    "into it.\n"
    "3. Five transitions — prefer deprecate / split over consolidate when "
"their preconditions are met. Pick the first applicable path; do not "
"skip ahead.\n"
    "   a. DEPRECATE (preferred over consolidate whenever a better "
"umbrella already exists). A single better-named umbrella or newer "
"skill covers the same domain and the narrow skill adds NO unique "
"content of its own (no paragraph, example, or template the "
"umbrella currently lacks):\n"
    "      • skill_manage action=\"deprecate\" with replaced_by=<umbrella>\n"
    "      • The original SKILL.md stays on disk for URL stability, "
"with `replaced_by` pointing at the umbrella; routing and cron "
"references follow the umbrella.\n"
    "      • DECISION RULE — for any narrow sibling of an existing "
"umbrella, ask: \"does the narrow skill have a unique paragraph, "
"example, or template the umbrella currently lacks?\". If NO, "
"use deprecate replaced_by=<umbrella>. If YES, fall through to "
"path c (MERGE INTO EXISTING UMBRELLA).\n"
    "      • Do NOT use action=\"delete\" absorbed_into=<umbrella> for "
"the no-unique-content case — that archives the SKILL.md off disk; "
"pick deprecate instead so the file survives with a pointer.\n"
    "   b. SPLIT (preferred over consolidate when a single skill "
"clearly covers two unrelated domains):\n"
    "      • skill_manage action=\"split\" with split_into=[<rep1>, <rep2>]\n"
    "      • The original SKILL.md stays on disk as a stub pointing "
"at the replacements. Use this when the skill is currently a single "
"catch-all that should have been two narrower skills all along.\n"
    "   c. MERGE INTO EXISTING UMBRELLA — one skill in the cluster is "
"already broad enough to be the umbrella (example: `pr-triage-"
"salvage` for the PR review cluster), AND the sibling has UNIQUE "
"content (paragraph / example / template) the umbrella currently "
"lacks. Patch the umbrella to add a labeled section, then archive "
"the sibling:\n"
    "      • skill_manage action=\"delete\" absorbed_into=<umbrella>\n"
    "      • Only use this when path a (deprecate) is wrong AND you "
"have real content to graft in. Do not use it as a generic \"remove "
"this duplicate\" lever — that is deprecate’s job (path a).\n"
    "   d. CREATE A NEW UMBRELLA SKILL.md — no existing member is broad "
...
"labeled subsections. Archive the now-absorbed narrow siblings:\n"
    "      • skill_manage action=\"delete\" absorbed_into=<new-umbrella>\n"
    "   e. DEMOTE TO REFERENCES/TEMPLATES/SCRIPTS — a sibling has "
...
"(verification scripts, fixture generators, probes)\n"
    "      Then archive the old sibling (action=\"delete\" absorbed_into=\"\") "
"using `terminal` with `mkdir -p ~/.hermes/skills/<umbrella>/references/ "
"&& mv ... <umbrella>/references/<topic>.md` (or templates/ / scripts/).\n"
    "   Record split and deprecate decisions in your human-readable "
"summary AND, where the underlying tool plumbing supports it, in "
"the structured summary block (see \"Expected output\" below).\n\n"
    ...
    "and look for the NEXT umbrella opportunity, but stop when the "
    "remaining candidates are already class-level umbrellas or "
    "narrow-but-genuinely-distinct siblings — quality over quantity.\n\n"
    "Your toolset:\n"
    ...
    "pass that burns iterations to hit an arbitrary archive count — "
    "judge each merge on its merits, not on a quota. Stop when the next "
    "candidate you would touch has no clear umbrella to fold into.\n\n"
