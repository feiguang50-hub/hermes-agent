...
CURATOR_REVIEW_PROMPT = (
    ...
    "   d. SPLIT or DEPRECATE instead of consolidating — the skill "
    "is doing work the curator has decided to retire, but the right "
    "outcome is a lifecycle transition, not an archive:\n"
    "      • skill_manage action=\"split\" with split_into=[...] — "
    "the skill covers TWO unrelated domains and you are creating "
    "(or have just created) replacement skill files for each domain. "
    "The original SKILL.md stays on disk as a stub pointing at the "
    "replacements. Use this when the skill is currently a single "
    "catch-all that should have been two narrower skills all along.\n"
    "      • skill_manage action=\"deprecate\" with replaced_by=... "
    "— a single better-named umbrella or newer skill covers the same "
    "domain. The original file stays on disk for URL stability. Use "
    "this when the skill was once distinct but has been superseded "
    "by something cleaner.\n"
    "      Use action=delete (archive) only when the content is "
    "genuinely obsolete or fully absorbed and no replacement skill "
    "is being created. Record split and deprecate decisions in your "
    "human-readable summary AND, where the underlying tool plumbing "
    "supports it, in the structured summary block (see \"Expected "
    "output\" below).\n\n"
